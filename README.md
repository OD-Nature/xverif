...
- [`xberif`](skills/xberif/SKILL.md)：维护验证项目 LLM wiki 的持续记忆 skill，避免 agent 每次 session 从 0 理解项目。
...
简单说：`xdebug` 负责“事实从哪里来、某时刻发生了什么”，`xbit` 负责“这些值按 SystemVerilog 规则算出来到底是多少”，`xentry` 负责“这个 entry 的 bit 域段按配置切出来是什么”，`xloc` 负责“这条 log 在哪个文件的哪一行，但只在需要时才查”，`xberif` 负责“把验证环境、DUT 功能、workflow、debug 入口等知识编译进持续 LLM wiki”，`xsva` 负责”assertion 的 temporal 语义先降成 IR，再解释给人和 agent”，`xcov` 负责“coverage database 里哪些 scope/object/bin 已覆盖或未覆盖，并给出源码 evidence”，`xverif-mcp` 负责”把确定性工具统一暴露给 AI agent 的 MCP 协议入口”，`xeda-runner` 负责”在预配置的安全白名单内执行 EDA 命令，先 init 缓存环境再 run 阻塞执行”。
...
`xberif` 是芯片验证持续记忆 skill。它要求 agent 通过 `XBERIF_WIKI_DIR` 找到当前 session 的 LLM wiki，从 `index.md`、concept 页面、反向索引和 `rg` 逐步查询验证环境、DUT 功能、接口、testbench、workflow、debug 入口等信息，并把新的稳定发现编译回 wiki。
...
```bash
export XBERIF_WIKI_DIR=/path/to/project/wiki
python skills/xberif/scripts/validate_xberif_wiki.py
```

完整说明见 [`skills/xberif/SKILL.md`](skills/xberif/SKILL.md)。
...
完整说明见 [`xcov/README.md`](xcov/README.md)，skill 见 [`skills/xverif/SKILL.md`](skills/xverif/SKILL.md) 和 [`skills/xverif/references/xcov.md`](skills/xverif/references/xcov.md)。
...
完整说明见 [`xeda_runner/README.md`](xeda_runner/README.md)，skill 见 [`skills/xverif/SKILL.md`](skills/xverif/SKILL.md) 和 [`skills/xverif/references/xeda-runner.md`](skills/xverif/references/xeda-runner.md)。
...
`xdebug` 需要可用的 Verdi/NPI 环境；`xbit` 只依赖 Python 标准库；`xberif` 是 skill 和标准库校验脚本，不再有 Python 包运行时。
...
- xverif agent skill：[`skills/xverif/SKILL.md`](skills/xverif/SKILL.md)
...
- xdebug agent reference：[`skills/xverif/references/xdebug/overview.md`](skills/xverif/references/xdebug/overview.md)
- xdebug JSON API 速查：[`skills/xverif/references/xdebug/json-api.md`](skills/xverif/references/xdebug/json-api.md)
- SDK-free xdebug wrapper：[`skills/xverif/references/sdk-free-xdebug/overview.md`](skills/xverif/references/sdk-free-xdebug/overview.md)
- MCP reference：[`skills/xverif/references/mcp/overview.md`](skills/xverif/references/mcp/overview.md)
...
- xberif 持续记忆 skill：[`skills/xberif/SKILL.md`](skills/xberif/SKILL.md)
...
- xeda-runner agent reference：[`skills/xverif/references/xeda-runner.md`](skills/xverif/references/xeda-runner.md)
