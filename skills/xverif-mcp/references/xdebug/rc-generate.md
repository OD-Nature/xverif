...
`rc.generate` 根据外部 JSON 配置生成 nWave `signal.rc`。MCP 场景下先用 `xverif_debug_session_open(fsdb=...)` 打开包含 FSDB 的 session，再通过 `xverif_debug_query(session_id=..., action="rc.generate")` 校验信号是否存在、marker 时间是否合法，然后写出 rc 文件。
