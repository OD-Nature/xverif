...
| action | status | resource | purpose | how it works | objective | args contract |
| --- | --- | --- | --- | --- | --- | --- |
| `session.close` | stable | session | 关闭指定 session。 | 释放该 session 的 runtime 资源并从管理器移除。 | 结束不再使用的调试上下文；MCP 常规关闭用 `xverif_debug_session_close`。 | native required: target.session_id |
...
| `session.kill` | stable | session | 强制移除指定 session。 | 按 native `target.session_id` 清理记录和关联资源。 | 处理异常残留 session；MCP 侧优先先用 session list/close。 | native required: target.session_id |
