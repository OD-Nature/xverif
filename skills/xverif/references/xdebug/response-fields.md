...
| `summary.truncated` | boolean | 是否截断 |
| `data` | object | unified engine design handler 返回 payload |
| `meta.truncated` | boolean | 同 payload truncated |

`data` 常见字段由 unified engine design handler 返回，可能包含：`query`、`ports[]`、`modport_ports[]`、`connections[]`、`instances[]`、`interfaces[]`、`truncated`、`error`。
