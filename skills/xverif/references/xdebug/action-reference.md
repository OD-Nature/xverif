...
- `value.at` / `value.batch_at`：`clock` 可选；省略时直接读取 `time` 的 FSDB 最终值并返回 `sampling_mode:"raw_time"`，传入时保留 clock-sampled 上下文。无 `clock` 时不得传 `edge` / `sample_point`。用 `args.value_format:"hex"|"bin"|"dec"` 控制响应显示，默认 hex；所有值带 `'h` / `'b` / `'d` 前缀。decimal 遇 X/Z 会明确报告 binary effective format。aggregate 根和 `format:"array_indexed"` 当前统一返回 `UNSUPPORTED_AGGREGATE_QUERY`；应传入最终 leaf signal，xdebug 不猜测 indexed path。
...
| `trace.active_driver_chain` | stable | combined | 展开 active driver 链。 | 从 signal/time 递归追溯 active driver；深度限制写 top-level `limits.max_depth`。遇到多个 active assignments 或多个 RHS sources 时在精确 `active_time` 返回前后值证据并停止。因 max_depth 停止时读取 `data.depth_frontiers` 与 `suggested_next_actions`，可从 frontier 续查或提高深度重跑。 | 给出跨级根因链及 ambiguous RHS 现场。 | required: signal, time<br>do not use `args.depth` or `args.clk_period` |
| `trace.x` | experimental | combined | 从指定 signal/time 追溯 X。 | 先确认查询值含 X，再按 DFS 同等追踪含 X 的 RHS/control；每条分支每一跳重新定位连续 X 区间起点并可多次向更早时间推进。深度停止时返回可直接续查的 frontier。 | 找到一个或多个 X 候选源、不可见边界或受限现场。 | required: signal, time<br>optional: value_format<br>limits: max_depth, max_nodes, max_time_steps, max_trace_signals, max_chains(default 8) |
