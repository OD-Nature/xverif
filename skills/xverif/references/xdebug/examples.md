...
需要 SDK-free wrapper 或 raw CLI 时，把同一 `action/args/limits/output` 放进 [json-api.md](json-api.md) 的原生 xdebug request envelope。
...
- 需要证明异常 transaction/beat 时，优先用 `axi.query` 的 `query.index` / `query.limit` 缩小，或用 `axi.export` 做批量导出。
...
`apb.query` / `axi.query` 的第 N 个 transfer/transaction 使用 1-based `args.query.index`；返回前 N 条使用 `args.query.limit`。不要写旧 `args.num`，也不要把数量限制猜成 `args.limit`。
