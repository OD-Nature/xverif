...
public:
    ...
    Json run(const Json& request, EngineActionContext& ctx) const override {
        ...
        if (!error.empty()) {
            if (!xdebug_waveform::g_axi_analyzer.last_cache_error().empty())
...
std::unique_ptr<EngineActionHandler> make_axi_request_response_pair_handler() {
    return std::unique_ptr<EngineActionHandler>(new AiActionHandler("axi.request_response_pair", false, true));
}
