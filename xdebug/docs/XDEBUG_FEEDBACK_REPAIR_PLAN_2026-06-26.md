...
- `trace.expand` 和 `trace graph view` 同时输出 graph `edges` 与 `trace.dependency_edges`，
...
| `trace.expand`、`trace graph view`、`trace explanation view` | handler 返回嵌套 `summary` 后又被 envelope 提升 | handler 返回顶层标量，由 envelope 生成 public summary |
