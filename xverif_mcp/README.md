...
xdebug backend 启动等待主要看
`XDEBUG_WAVEFORM_SESSION_START_TIMEOUT_SEC`、`XDEBUG_DESIGN_SESSION_START_TIMEOUT_SEC`。
...
"env": {
  ...
  "XDEBUG_WAVEFORM_SESSION_START_TIMEOUT_SEC": "300",
  "XDEBUG_DESIGN_SESSION_START_TIMEOUT_SEC": "300"
}
```
...
| `XDEBUG_WAVEFORM_SESSION_START_TIMEOUT_SEC` | xdebug waveform daemon 启动等待超时（默认 180s） |
| `XDEBUG_DESIGN_SESSION_START_TIMEOUT_SEC` | xdebug design daemon 启动等待超时（默认 180s） |
