...
- **xbit / xentry / xloc / xsva**：stateless in-process adapter，由 MCP server 直接 import 对应 Python API。
...
| Verdi/VCS | **O-2018.09-SP2** 或 **V-2023.12-SP2** |
| Profile | `XVERIF_EDA_PROFILE=verdi-2018|verdi-2023` |
| xcov backend | 2018使用 native worker；2023使用 Python `pynpi.cov` |

启动 MCP前建议先在仓库根目录运行对应的 `make self-test-2018` 或
`make self-test-2023`。其他 EDA版本目前未验证。
...
- `<verdi-profile>`：`verdi-2018` 或 `verdi-2023`，必须与安装目录一致
...
AI MCP client
  ...
  -> In-process adapters (xbit/xentry/xloc/xsva)
...
| `XVERIF_EDA_PROFILE` | EDA兼容 profile：`verdi-2018` 或 `verdi-2023` |
| `XVERIF_XCOV_BACKEND` | coverage backend显式覆盖：`native` 或 `python`；通常由 profile自动选择 |
