...
- Verdi 2018可分发自检：`make self-test-2018`
- Verdi 2023可分发自检：`make self-test-2023`
...
- 错误现象：同步上游后运行 `make -C xdebug test-fast`，`test_request_contract` 未拒绝未知顶层字段。
- 误判原因：执行前没有检查到 shell 的 `XVERIF_HOME` 仍指向另一个旧版仓库，导致新二进制加载了旧仓库 schema。
- 以后规则：运行依赖仓库内 schema、examples、engine 或工具路径的测试前，必须确认 `XVERIF_HOME` 指向当前工作树；多工作树并存时在命令中显式设置当前仓库路径。
...
- 错误现象：在沙箱内运行 `xverif_mcp/tests/test_loop_wrapper_uds.py`，8 个用例因 UDS `bind` 返回 `EPERM` 而失败。
- 误判原因：测试使用 fake backend，但忽略了 wrapper 本身仍会创建真实 Unix domain socket。
- 以后规则：凡是 MCP/SDK-free wrapper 测试会执行 UDS/TCP/file transport 的真实 bind 或进程通信，即使 backend 是 fake，也必须直接在沙箱外运行。

### 2026-07-11 环境错误复盘

- 错误现象：绕过 `tools/xcov` 直接运行原生 Dispatcher 验证真实 VDB时，native worker 因 NPI resource directory 不在 `LD_LIBRARY_PATH` 而初始化失败。
- 误判原因：为绕开 MCP startup timeout 改用 Dispatcher 时，遗漏了原 wrapper 负责的 Verdi NPI library 环境注入。
- 以后规则：直接调用 xcov Python Dispatcher/NPI backend 时，必须显式设置当前 profile 的 `VERDI_HOME` 和 NPI `LD_LIBRARY_PATH`；常规调用优先保留 `tools/xcov` wrapper。
