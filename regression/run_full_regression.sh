...
main() {
    ...
    if [[ -f ~/xif_agent/src/xif_pkg.sv ]] && command -v vcs >/dev/null 2>&1; then
    ...
    if [[ -f ~/axi_test/test/sim_run/tb.fsdb ]]; then
    ...
    if [[ -f ~/wave_tmp/waves.fsdb && -f "$ROOT/xdebug/tests/realdata/run_system_wave.py" ]]; then
